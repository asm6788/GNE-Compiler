    ...
    internal class Compiler
    {
        ...
        public class Parser : Compiler
        {
            ...
            public new void Variable(string source)
            {
                ...
                {
                    VariableTable.Add(parse[1], hash);
                }
                if (parse[4] == "창조")
                { 
                    generated.Insert(index, "static dynamic " + hash + " = null;");
                    source = source.Remove(0, source.IndexOf("이것이다") + 7);
                    variables.Add(new Variable("var", hash, parse[5].Remove(parse[5].Length - 1, 1).Trim()));
                    generated.Add(hash + " = new " + ParsedToCsharp(Operator.Parse(source)) + ";");
                }
                else
                {
                    generated.Insert(index, "static dynamic " + hash + " = null;");
                    ...
                    generated.Add(hash + " = " + ParsedToCsharp(Operator.Parse(source)) + ";");
                }
            }
